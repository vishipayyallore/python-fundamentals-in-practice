...
Official reference: [PEP 8 — Style Guide for Python Code](https://peps.python.org/pep-0008/#introduction)
...
### Rule 3B — Use clear indentation and readable wrapped lines

Python uses indentation as part of the language, so visual structure matters.
When one call needs multiple lines, line up the continuation clearly instead of letting the values drift to the left edge.
...
The first version makes the grouping obvious. The second version forces the reader to work harder.
...
**Line length for comments:** PEP 8 recommends keeping comment lines to about **72 characters** so they stay readable in editors and code review. If a `# Why:` note runs long, split it across two comment lines rather than stretching one line to the edge of the screen.
