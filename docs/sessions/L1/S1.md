...
graph TD
  ...
  classDef start fill:#eef6ff,stroke:#86a9cf,stroke-width:2px,color:#1f2a37
  classDef process fill:#f5faff,stroke:#a8bfdc,stroke-width:2px,color:#1f2a37
  classDef runtime fill:#e7f1ff,stroke:#7f9fc7,stroke-width:2px,color:#1f2a37
  classDef result fill:#ddeeff,stroke:#6f90bd,stroke-width:2px,color:#1f2a37
...
**Visual reference (Mermaid):** same pipeline as the ASCII diagram above.

```mermaid
flowchart TD
    A["Source file (.py)"] --> B["Compiler (syntax check)"]
    B --> C["Bytecode (.pyc)"]
    C --> D["Python Virtual Machine"]
    D --> E["Output / effect"]
```

Optional screenshot (instructor assets): `docs/images/S1/Py_Source_ByteCode.PNG` — see `docs/images/README.md` if you maintain local PNGs.
...
**Next ([Session 2: Variables & Data Types](S2.md)):** you **name** data with variables and learn **data types** and **conversion** (`int`, `str`, etc.) so programs can **store and use** input correctly.
...
**What you typically see in the REPL:**
...
`help()` opens Python's built-in documentation browser. `help(print)` narrows the view to one function — useful when you forget parameter names like `sep` or `end`.

Optional screenshots (instructor assets): `docs/images/S1/Help_V1.PNG`, `docs/images/S1/Help_V2.PNG` — see `docs/images/README.md`.
